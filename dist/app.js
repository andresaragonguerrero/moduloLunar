...
form === null || form === void 0 ? void 0 : form.addEventListener('submit', function (event) {
    ...
    const inputName = document.getElementById('mineralName');
    const inputHardness = document.getElementById('hardness');
    const inputGrainSize = document.getElementById('grainSize');
    const inputCrystalSize = document.getElementById('crystalSize');
    const inputFormationTemperature = document.getElementById('formationTemperature');
    const inputOrigin = document.querySelector('input[name="mineralOrigin"]:checked');
    const inputClassification = document.querySelector('input[name="classification"]:checked');
    const inputTexture = document.querySelector('input[name="texture"]:checked');
    // Obtener valores
    const identifier = inputIdentifier.value;
    const mineralName = inputName.value;
    const mineralOrigin = (inputOrigin === null || inputOrigin === void 0 ? void 0 : inputOrigin.value.toLowerCase()) || '';
    ...
    const classification = (inputClassification === null || inputClassification === void 0 ? void 0 : inputClassification.value) || '';
    const formationTemperature = Number.parseFloat(inputFormationTemperature.value);
    const texture = (inputTexture === null || inputTexture === void 0 ? void 0 : inputTexture.value) || '';
    const structure = '';
    const grainForm = '';
    // Crear objeto Mineral
    const mineral = new Mineral(identifier, mineralName, mineralOrigin, hardness, grainSize, classification, crystalSize, formationTemperature, structure, grainForm, texture);
    // Validación
    const validator = ValidatorFactory.getValidator(mineralOrigin);
    const isValid = validator ? validator.validate(mineral) : false;
    // Formateo de salida
    const selectOutput = document.getElementById('outputFormat');
    const selectedFormat = selectOutput.value;
    let formatter;
    if (selectedFormat === 'american')
        formatter = new AmericanFormatter();
    else
        formatter = new EuropeanFormatter();
    ...
    form.innerHTML = isValid ? `<div class="output-container">${smiley}<p class="output-info">${output}</p></div>`
        : `<div class="output-container">${disappointed}<p class="output-info">${output}</p></div>`;
});
/**
 * Identificador: AA0000AA (cumple LLDDDDLL)

Nombre: Granito

Origen: Ígneas (elige el radio button)

Dureza: 6

Tamaño de grano: 31 (muy grueso)

Clasificación: construction

Tamaño de cristal: 5

Temperatura de formación: 80
